...
// Cross-runtime SQL END-TO-END capture parity: drive the REAL .NET Npgsql adapter against a live Postgres
...
// Cross-runtime MySQL END-TO-END capture parity: drive the REAL .NET MySqlConnector tracking wrapper against a
// live MySQL (KRON_MYSQL) and dump the emitted RequestResponseLog pairs; the Java side drives the same generic
...
void CaptureHtmlBlankName()
{
    var start = new DateTime(2024, 1, 15, 10, 0, 0, DateTimeKind.Utc);
    var end = new DateTime(2024, 1, 15, 10, 0, 5, DateTimeKind.Utc);
    // A scenario with an empty DisplayName + a step with empty text — degenerate but deterministic: the
    // anchor slug, the empty <summary>/copy-button, and the step rendering must match .NET byte-for-byte.
    var scenario = new Scenario
    {
        Id = "s1", DisplayName = "", IsHappyPath = true,
        Result = ExecutionResult.Passed, Duration = TimeSpan.FromMilliseconds(100),
        Steps = [ new ScenarioStep { Keyword = "Given", Text = "", Status = ExecutionResult.Passed, Duration = TimeSpan.FromMilliseconds(10) } ]
    };
    var feature = new Feature { DisplayName = "Shop", Scenarios = [scenario] };
    var diagrams = Array.Empty<DefaultDiagramsFetcher.DiagramAsCode>();
    var path = ReportGenerator.GenerateHtmlReport(
        diagrams, [feature], start, end, null, "report-blankname.html", "Kronikol Run", includeTestRunData: false);
    var content = File.ReadAllText(path).ReplaceLineEndings("\n");
    File.WriteAllText(Path.Combine(outDir, "report-blankname.html"), content);
    Console.WriteLine($"=== report-blankname.html ({content.Length} chars) ===");
}
...
{
    var cs = Environment.GetEnvironmentVariable("KRON_MYSQL")
        ?? "Server=localhost;Port=13306;User ID=kron;Password=pw;Database=test";
    using var raw = new MySqlConnector.MySqlConnection(cs);
    raw.Open();
    using (var drop = raw.CreateCommand()) { drop.CommandText = "DROP TABLE IF EXISTS orders"; drop.ExecuteNonQuery(); }
    var tracked = Kronikol.Extensions.MySqlConnector.MySqlConnectionExtensions.WithTestTracking(raw,
        new Kronikol.Extensions.MySqlConnector.MySqlTrackingOptions
        {
            ServiceName = "OrdersDb",
            CurrentTestInfoFetcher = () => ("MyTest", "t1"),
        });
    ...
    void Exec(string sql) { using var c = tracked.CreateCommand(); c.CommandText = sql; c.ExecuteNonQuery(); }
    void Query(string sql) { using var c = tracked.CreateCommand(); c.CommandText = sql; using var r = c.ExecuteReader(); while (r.Read()) { } }
    Exec("CREATE TABLE orders (id INT, name TEXT)");
    Exec("INSERT INTO orders (id, name) VALUES (1, 'a')");
    Query("SELECT * FROM orders WHERE id = 1");
    Exec("UPDATE orders SET name = 'b' WHERE id = 1");
    ...
    Kronikol.Tracking.RequestResponseLogger.Clear();
    raw.Close();
    File.WriteAllText(Path.Combine(outDir, "mysql-interactions.txt"), sb.ToString().ReplaceLineEndings("\n"));
    Console.WriteLine($"=== mysql-interactions.txt ===\n{sb}");

    static string NormalizeHost(string uri) =>
        System.Text.RegularExpressions.Regex.Replace(uri, @"^([a-z]+://)[^/]+", "$1HOST");
}

void CaptureSqlInteractions()
{
    var cs = Environment.GetEnvironmentVariable("KRON_PG")
        ?? "Host=localhost;Port=15432;Username=kron;Password=pw;Database=test";
    using var raw = new Npgsql.NpgsqlConnection(cs);
    raw.Open();
    using (var drop = raw.CreateCommand()) { drop.CommandText = "DROP TABLE IF EXISTS orders"; drop.ExecuteNonQuery(); }
    var tracked = Kronikol.Extensions.Npgsql.NpgsqlConnectionExtensions.WithTestTracking(raw,
        new Kronikol.Extensions.Npgsql.NpgsqlTrackingOptions
        {
            ServiceName = "OrdersDb",
            CurrentTestInfoFetcher = () => ("MyTest", "t1"),
        });
    ...
    Kronikol.Tracking.RequestResponseLogger.Clear();
    raw.Close();
    File.WriteAllText(Path.Combine(outDir, "sql-interactions.txt"), sb.ToString().ReplaceLineEndings("\n"));
    Console.WriteLine($"=== sql-interactions.txt ===\n{sb}");

    // Normalise the env-specific host[:port] segment of <scheme>://<host>/<db>/... to a fixed token (keeping
